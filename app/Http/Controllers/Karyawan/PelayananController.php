...

{
    ...
    public function store(AddOrderRequest $request)
    {
      try {
        ...
        $total_harga            = $order->harga + $order->harga;
...
{
    $list_harga_1 = harga::select('id','harga')
        ->where('user_id', Auth::user()->id)
        ->where('id', $request->id)
        ->get();
    $list_harga_2 = harga::select('id','harga')
        ->where('user_id', Auth::user()->id)
    ...
    $select .= '
        <div class="form-group has-success">
            ...
            <select id="id1" class="form-control" name="harga_1" value="harga_1">
    ';
    foreach ($list_harga_1 as $studi) {
        $select .= '<option value="'.$studi->harga.'" data-id="'.$studi->id.'">'.'Rp. ' .number_format($studi->harga,0,",",".").'</option>';
    }
    $select .= '
            </select>
        </div>
        <div class="form-group has-success">
            <label for="id2" class="control-label" name="harga_2" value="harga_2">Harga 2</label>
            <select id="id2" class="form-control" name="harga_2" value="harga_2">
    ';
    foreach ($list_harga_2 as $studi) {
        $select .= '<option value="'.$studi->harga.'" data-id="'.$studi->id2.'">'.'Rp. ' .number_format($studi->harga,0,",",".").'</option>';
    }
    $select .= '
            </select>
        </div>
    ...
    $select .= '<p>Total Harga: <span id="total_harga">0</span></p>';
    $select .= '<button id="btnHitung" type="button" class="btn btn-primary">Hitung Total Harga</button>';

    $select .= '
        <script>
            function calculateTotalPrice() {
                var price1 = parseInt($("#id1").val());
                var price2 = parseInt($("#id2").val());
